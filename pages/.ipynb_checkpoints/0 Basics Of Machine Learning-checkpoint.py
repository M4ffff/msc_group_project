...
st.subheader("Demystifying :rainbow[key] terminology and concepts:")
st.markdown("-	**:blue[Artificial Intelligence (AI)]**: The broad field of creating systems capable of intelligent behaviour, from playing chess to making medical diagnoses.")
st.markdown("-	**:blue[Machine Learning (ML)]**: A subset of AI focused on developing algorithms that enable computers to learn patterns from data and make predictions.")
st.markdown("-  **:blue[Supervised Learning]**: A type of ML where the model is trained on labelled data, learns patterns, and makes predictions on new 'unseen' data.")
st.markdown("-  **:blue[Unsupervised Learning]**: A type of ML which involves the model finding patterns in data without labelled outputs, uncovering hidden relationships.")
st.markdown("-	**:blue[Deep Learning]**: A specialised branch of ML that uses neural networks with many layers to solve complex problems like image and speech recognition.")
st.markdown("-  **:blue[Shallow Learning]**: A branch of ML which uses models that typically require manual feature engineering and do not have hidden layers, such as Logistic Regression.")
...
st.markdown("This web app is structured into several sections, each focusing on different essential ML concepts.")

with st.expander("**:blue[Section One:]** Supervised Learning"):
    st.markdown('''
    - Linear Regression
...
with st.expander("**:blue[Section Two:]** Unsupervised Learning"):
    st.markdown('''
...
with st.expander("**:blue[Section Three:]** Neural Networks"):
    st.markdown('''
